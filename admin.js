// Uniquement la configuration sécurisée
const SECURE_CONFIG = {
    MOT_DE_PASSE: "DC3NNAYKEFEV"
};

// Le reste utilise CONFIG du fichier config.js

document.getElementById('login-form').addEventListener('submit', function(e) {
    e.preventDefault();
    const password = document.getElementById('admin-password').value;
    
    if (password === SECURE_CONFIG.MOT_DE_PASSE) {
...
async function supprimerPoeme(id) {
...
async function modifierPoeme(id) {
    try {
        const response = await fetch(`https://api.github.com/repos/${CONFIG.REPO_OWNER}/${CONFIG.REPO_NAME}/contents/${CONFIG.FILE_PATH}`);
        const fileData = await response.json();
        const content = JSON.parse(atob(fileData.content));
        ...
        if (poemeEnEdition) {
            ...
            document.getElementById('poem-genre').value = poemeEnEdition.genre || '';
